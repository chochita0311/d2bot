...
def find_minimap_waypoint_with_scouting(session, capture):
    ...
    for index, point in enumerate(session.MINIMAP_SCOUT_POINTS, start=1):
        session.events.put(
            session.event_class(
                "info", f"Summoner: waypoint marker is faint from spawn view, scouting right ({index}/{len(session.MINIMAP_SCOUT_POINTS)})."
            )
        )
        click_relative_ratio(session, capture, point[0], point[1], apply_jitter=False)
        sleep_range(session, *session.SCOUT_MOVE_SETTLE)
        match = wait_for_optional_any_template(
            session, capture, session._act1_map_templates, session.MAP_THRESHOLD, session.MAP_SCAN_TIMEOUT
        )
...
def refresh_visible_waypoint(session, capture, fallback):
    packet = capture.grab()
    refreshed = locate_best_template(
...
def find_world_waypoint_with_scouting(session, capture):
    best_match = None
    initial = wait_for_optional_any_template(
        session,
        capture,
        [session._act1_waypoint_hover_template, session._act1_waypoint_template],
        session.WAYPOINT_THRESHOLD,
        session.WAYPOINT_SCAN_TIMEOUT,
    )
    if initial is not None:
        return initial
    best_match = locate_best_template(
        session, capture.grab().frame, [session._act1_waypoint_hover_template, session._act1_waypoint_template], 0.0
    )
    ...
    session.events.put(
        session.event_class("info", "Summoner: waypoint not visible yet, holding upward movement from the Diablo window center.")
    )
    ...
    if match is not None:
        ...
        session._last_detection_route = "vertical_sweep"
        return match
    fallback = locate_best_template(
        session, capture.grab().frame, [session._act1_waypoint_hover_template, session._act1_waypoint_template], 0.0
    )
    if fallback is not None and (best_match is None or fallback.score > best_match.score):
        best_match = fallback
    session.events.put(
        session.event_class("info", "Summoner: waypoint not visible yet, holding downward movement from the Diablo window center.")
    )
    ...
    session.events.put(
        session.event_class("info", "Summoner: vertical sweep missed; holding rightward movement from the Diablo window center.")
    )
...
def open_act1_waypoint_list(session, capture):
    for attempt in range(1, session.OPEN_ATTEMPTS + 1):
        ...
        session.events.put(
            session.event_class(
                "info",
                f"Summoner: found the real waypoint object on screen (attempt {attempt}/{session.OPEN_ATTEMPTS}, score={target_match.score:.3f}).",
            )
        )
        approach_waypoint(session, capture, target_match)
        sleep_range(session, *session.CLICK_SETTLE)
        hover_match = wait_for_optional_template(
            session, capture, session._act1_waypoint_hover_template, session.WAYPOINT_HOVER_THRESHOLD, session.HOVER_WAIT_SECONDS
        )
...
def travel_to_arcane_sanctuary(session, capture, act1_panel) -> None:
    ...
    sleep_range(session, *session.ACTION_SLEEP)
    act2_panel = wait_for_waypoint_panel(
        session, capture, session._act2_list_panel_template, session.LIST_PANEL_THRESHOLD, 2.5, "Act 2 waypoint list"
    )
